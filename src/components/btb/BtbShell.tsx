...
export function BtbShell({ children, eyebrow, title }: { children: ReactNode; eyebrow?: string; title?: string }) {
  ...
  const content = useProgrammeContent("basic-training-blueprint-plus", !!user);
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 border-t border-border bg-background/95 backdrop-blur z-40" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        ...
        <ul className="grid grid-cols-5">
          {primary.map((it) => {
            ...
            return (
              <li key={it.to}>
                <Link to={it.to} className={`flex flex-col items-center gap-1 py-3 text-[10px] uppercase tracking-widest ${active ? "text-bone" : "text-foreground-muted"}`}>
                  <Icon className="h-4 w-4" strokeWidth={1.5} />
                  <span>{it.label}</span>
  ...
  return (
    <section className="min-h-[60vh] grid place-items-center px-5 py-20 bg-background">
      <div className="max-w-md text-center">
        ...
        <h1 className="font-display font-bold text-bone text-3xl tracking-tight uppercase">Sign in to access Basic Training Blueprint+</h1>
  ...
  return (
    <section className="min-h-[60vh] grid place-items-center px-5 py-20 bg-background">
      <div className="max-w-md text-center">
        ...
        <h1 className="font-display font-bold text-bone text-3xl tracking-tight uppercase">Basic Training Blueprint+ isn't in your library</h1>
        ...
        <div className="mt-8 flex justify-center gap-4">
          ...
          <Link to="/programmes/basic-training-blueprint-plus" className="h-11 px-6 inline-flex items-center bg-bone text-obsidian text-xs uppercase tracking-widest font-display">View product</Link>
        </div>
      </div>
    </section>
  );

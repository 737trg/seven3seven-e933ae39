...
import { CalendarDays, LineChart, BookOpen, User, Flame, Calculator, LayoutGrid, X } from "lucide-react";
...
import { mixedStore } from "@/lib/mixed/store";

const BASE = "/my-programmes/mixed";
const primary = [
...
export function MixedShell({ children, eyebrow, title }: { children: ReactNode; eyebrow?: string; title?: string }) {
  const pathname = useRouterState({ select: (r) => r.location.pathname });
  const { user, loading: authLoading } = useAuth();
  const { items, loading: entLoading } = useEntitlements(user?.id);
  const content = useProgrammeContent("mixed", !!user);

  useEffect(() => {
    mixedStore.configureUser(user?.id ?? null);
  }, [user?.id]);

  if (!authLoading && !user) return <GateSignIn />;
  if (!authLoading && !entLoading && !items.some((i) => i.slug === "mixed")) return <GateNoEntitlement />;
  ...
  return (
    <div className="min-h-dvh bg-background text-foreground flex">
      <aside className="hidden lg:flex flex-col w-[240px] shrink-0 border-r border-border bg-background sticky top-0 h-dvh px-6 py-8">
        <div className="mb-12">
          <Wordmark size="lg" />
          <p className="eyebrow mt-3">MIXED</p>
        </div>
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border bg-background/95 backdrop-blur" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        <div className="flex items-center justify-center gap-6 px-5 py-2 border-b border-border/60">
          {secondary.map((it) => {
            ...
            const Icon = it.icon;
            return (
              <Link key={it.to} to={it.to} className={`flex items-center gap-1.5 text-[10px] uppercase tracking-widest ${active ? "text-bone" : "text-foreground-muted"}`}>
  ...
  return (
    <section className="min-h-[60vh] grid place-items-center px-5 py-20 bg-background">
      <div className="max-w-md text-center">
        ...
        <h1 className="font-display font-bold text-bone text-3xl tracking-tight uppercase">Sign in to access MIXED</h1>
  ...
  return (
    <section className="min-h-[60vh] grid place-items-center px-5 py-20 bg-background">
      <div className="max-w-md text-center">
        ...
        <h1 className="font-display font-bold text-bone text-3xl tracking-tight uppercase">MIXED isn't in your library</h1>
        <p className="text-foreground-muted text-sm mt-4">Buy MIXED outright or join the Club to unlock it.</p>
        <div className="mt-8 flex justify-center gap-4">
          ...
          <Link to="/programmes/$slug" params={{ slug: "mixed" }} className="h-11 px-6 inline-flex items-center bg-bone text-obsidian text-xs uppercase tracking-widest font-display">View product</Link>
        </div>
      </div>
    </section>
  );

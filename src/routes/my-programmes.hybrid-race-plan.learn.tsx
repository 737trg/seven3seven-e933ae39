...
export const Route = createFileRoute("/my-programmes/hybrid-race-plan/learn")({
  head: () => ({ meta: [{ title: "HYBRID RACE PLAN — Learn" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: LearnPage,
});
...
function LearnPage() {
  const tabs = (HRP as any).learn?.tabs as Record<string, string[]> | undefined;
  ...
  return (
    <HrpShell eyebrow="Learn" title="Terms, standards, rules">
      ...
      <p className="mt-12 text-foreground-muted text-xs max-w-[60ch]">{HRP.independent_disclaimer}</p>
    </HrpShell>
  );
}
...
import { SEM27, isCore, sessionId, validationCounts } from "@/lib/sem2027/manifest";
import { useSem27Progress } from "@/lib/sem2027/progress";
...
export const Route = createFileRoute("/my-programmes/sem-2027/programme/")({
  head: () => ({ meta: [{ title: "S.E.M. 2027 — Programme" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: ProgrammePage,
});

function ProgrammePage() {
  const { user } = useAuth();
  const prog = useSem27Progress(user?.id);
  ...
  return (
    <Sem27Shell eyebrow="The plan" title="Programme">
              ...
              {w.sessions.map((s) => {
                ...
                return (
                  <li key={id} className={`py-4 flex items-center justify-between gap-4 ${hideOptional ? "opacity-60" : ""}`}>
                    ...
                    <Link to="/my-programmes/sem-2027/programme/s/$sessionId" params={{ sessionId: id }} className="tap press shrink-0 text-foreground-muted hover:text-bone inline-flex items-center justify-end gap-2 text-[10px] uppercase tracking-widest">

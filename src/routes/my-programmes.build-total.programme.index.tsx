...
import { TOTAL, isCore, sessionId, validationCounts } from "@/lib/total/manifest";
...
export const Route = createFileRoute("/my-programmes/build-total/programme/")({
  head: () => ({ meta: [{ title: "TOTAL — Programme" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: ProgrammePage,
});

function ProgrammePage() {
  const { user } = useAuth();
  const prog = useTotalProgress(user?.id);
  const counts = validationCounts();
  const corePct = Math.round((prog.coreCompleted / Math.max(1, counts.core)) * 100);

  return (
    <TotalShell eyebrow="The plan" title="Programme">
              ...
              {w.sessions.map((s) => {
                ...
                return (
                  <li key={id} className="py-4 flex items-center justify-between gap-4">
                    ...
                    <Link to="/my-programmes/build-total/programme/s/$sessionId" params={{ sessionId: id }} className="tap press shrink-0 text-foreground-muted hover:text-bone inline-flex items-center justify-end gap-2 text-[10px] uppercase tracking-widest">

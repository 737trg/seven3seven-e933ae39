...
import { SEM, isCore, sessionId, validationCounts } from "@/lib/sem/manifest";
...
function ProgrammePage() {
  const { user } = useAuth();
  const prog = useSemProgress(user?.id);
  ...
  return (
    <SemShell eyebrow="The plan" title="Programme">
              ...
              {w.sessions.map((s) => {
                ...
                return (
                  <li key={id} className={`py-4 flex items-center justify-between gap-4 ${hideOptional ? "opacity-60" : ""}`}>
                    ...
                    <Link to="/my-programmes/sem-2026/programme/s/$sessionId" params={{ sessionId: id }} className="tap press shrink-0 text-foreground-muted hover:text-bone inline-flex items-center justify-end gap-2 text-[10px] uppercase tracking-widest">

...
import { findSession, blockId } from "@/lib/sem2027/manifest";
import { useSem27Profile } from "@/lib/sem2027/store";
import { Flame } from "lucide-react";

export const Route = createFileRoute("/my-programmes/sem-2027/programme/s/$sessionId")({
  head: () => ({ meta: [{ title: "S.E.M. 2027 — Session" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: SessionPage,
});

function SessionPage() {
  const { sessionId: sid } = useParams({ from: "/my-programmes/sem-2027/programme/s/$sessionId" });
  const ref = findSession(sid);
  const profile = useSem27Profile();

  if (!ref) {
    return (
      <Sem27Shell eyebrow="Session" title="Not found">
        <p className="text-foreground-muted">That session does not exist in this programme.</p>
        <Link to="/my-programmes/sem-2027/programme" className="mt-6 inline-block text-bone underline">Back to programme</Link>
      </Sem27Shell>
    );
  }
  ...
  return (
    <Sem27Shell eyebrow={`Week ${week.week} · ${week.phase}`} title={session.title}>

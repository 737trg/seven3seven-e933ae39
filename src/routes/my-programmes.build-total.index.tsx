...
import { totalStore, useTotalStarted } from "@/lib/total/store";
...
export const Route = createFileRoute("/my-programmes/build-total/")({
  head: () => ({
    meta: [
      { title: "TOTAL — SEVEN3SEVEN" },
      { name: "robots", content: "noindex, nofollow" },
      { name: "description", content: "TOTAL — programme cover." },
    ],
  }),
  component: TotalCover,
});

function TotalCover() {
  const { user, loading: authLoading } = useAuth();
  const { items, loading: entLoading } = useEntitlements(user?.id);
  const started = useTotalStarted();
  const navigate = useNavigate();

  const entitled = !authLoading && !entLoading && items.some((i) => i.slug === "build-total");
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              ...
              <button
                onClick={() => {
                  ...
                  if (!started.started) totalStore.markStarted();
                  navigate({ to: "/my-programmes/build-total/today" });
                }}
              ...
              <PdfDownloadLink slug="build-total" href={pdfAsset.url} label="Download PDF" />
              {!entitled && !entLoading && (

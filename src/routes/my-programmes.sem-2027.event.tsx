...
import { useSem27Profile, sem27Store } from "@/lib/sem2027/store";
import { useState } from "react";

export const Route = createFileRoute("/my-programmes/sem-2027/event")({
  head: () => ({ meta: [{ title: "S.E.M. 2027 — Race day" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: EventPage,
});
...
function EventPage() {
  ...
  return (
    <Sem27Shell eyebrow="Race day" title="Event plan">
      ...
      <button onClick={() => { /* placeholder — local plan persistence reserved */ sem27Store.saveProfile({}); }} className="hidden" />
    </Sem27Shell>
  );
}
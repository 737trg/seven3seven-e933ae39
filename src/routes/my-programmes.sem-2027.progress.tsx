...
import { useSem27Profile, currentSem27Week } from "@/lib/sem2027/store";
import { validationCounts, SEM27, isCore, sessionId } from "@/lib/sem2027/manifest";

export const Route = createFileRoute("/my-programmes/sem-2027/progress")({
  head: () => ({ meta: [{ title: "S.E.M. 2027 — Progress" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: ProgressPage,
});

function ProgressPage() {
  const { user } = useAuth();
  const p = useSem27Progress(user?.id);
  ...
  return (
    <Sem27Shell eyebrow="Performance" title="Progress">

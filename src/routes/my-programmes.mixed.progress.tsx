...
import { useMixedProfile, currentMixedWeek } from "@/lib/mixed/store";
import { validationCounts, MIXED, isCore, sessionId } from "@/lib/mixed/manifest";

export const Route = createFileRoute("/my-programmes/mixed/progress")({
  head: () => ({ meta: [{ title: "MIXED — Progress" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: ProgressPage,
});

function ProgressPage() {
  const { user } = useAuth();
  const p = useMixedProgress(user?.id);
  ...
  const tests = (MIXED.progress as { tests?: string[] }).tests ?? [];

  return (
    <MixedShell eyebrow="Performance" title="Progress">

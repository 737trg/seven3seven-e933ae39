...
export const Route = createFileRoute("/my-programmes/mixed/learn")({
  head: () => ({ meta: [{ title: "MIXED — Learn" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: LearnPage,
});
...
function LearnPage() {
  ...
  return (
    <MixedShell eyebrow="Coaching" title="Learn">
        ...
        <div className="space-y-10">
          <section className="border border-border p-5">
            ...
            <ul className="text-bone text-sm space-y-1.5">
              {MIXED.entry_gate.map((g) => <li key={g}>· {g}</li>)}
            </ul>
          </section>
          {MIXED.foundation.map((f) => (
          ...
          <p className="text-foreground-muted text-xs max-w-[70ch] border-t border-border pt-5">{MIXED.safety}</p>
        </div>
      )}
      ...
      {tab === "scaling" && (
        <div className="space-y-8">
          <p className="text-bone text-sm max-w-[70ch]">{String((MIXED.rx_scaled_policy as Record<string, unknown>).rule ?? "")}</p>
          <p className="text-signal text-xs uppercase tracking-[0.18em]">{String((MIXED.rx_scaled_policy as Record<string, unknown>).no_shame_copy ?? "")}</p>
      ...
      {tab === "equipment" && (
        <div className="space-y-6">
          <div>
            ...
            <ul className="text-bone text-sm space-y-1.5">
              {((MIXED.equipment as { primary?: string[] }).primary ?? []).map((e) => <li key={e}>· {e}</li>)}
            </ul>
          </div>
          <p className="text-foreground-muted text-sm max-w-[70ch]">{String((MIXED.equipment as Record<string, unknown>).substitution_principle ?? "")}</p>
          <ul className="text-bone text-sm space-y-3">
            {(((MIXED.equipment as Record<string, unknown>).substitutions as { if_missing: string; use: string; keep_the_same?: string }[]) ?? []).map((s) => (
      ...
      {tab === "reference" && (
        <div className="space-y-8">
          <pre className="text-foreground-muted text-xs whitespace-pre-wrap font-sans max-w-[80ch]">
            {JSON.stringify(MIXED.training_reference, null, 2)
              .replace(/[{}"[\],]/g, "")
              .replace(/_/g, " ")
              .split("\n")
              .filter((l) => l.trim())

...
import { SEM27, allSessions, isCore, sessionId } from "@/lib/sem2027/manifest";
import { sem27Store, useSem27Profile, useSem27Readiness, currentSem27Week, type Sem27Readiness } from "@/lib/sem2027/store";
...
export const Route = createFileRoute("/my-programmes/sem-2027/today")({
  head: () => ({ meta: [{ title: "S.E.M. 2027 — Today" }, { name: "robots", content: "noindex, nofollow" }] }),
  component: TodayPage,
});

function TodayPage() {
  const profile = useSem27Profile();
  const readiness = useSem27Readiness();
  const schedule = useScheduleOverrides("sem-2027");
  const week = currentSem27Week(profile.startDate) ?? 1;
  const wk = SEM27.weeks.find((w) => w.week === week)!;
  ...
  const readinessValue = (readiness[currentId] ?? "ready") as Sem27Readiness;
  const adj = current.readiness_adjustment ?? {};

  return (
    <Sem27Shell eyebrow={`Week ${wk.week} · ${wk.phase}`} title={profile.displayName ? `Good work, ${profile.displayName}.` : "Today."}>
      {!profile.setupComplete && (
        <div className="mb-8 border border-signal/40 bg-signal/10 p-5">
          ...
          <Link to="/my-programmes/sem-2027/profile" className="mt-3 inline-flex items-center gap-2 text-bone text-xs uppercase tracking-widest font-display">Open profile <ArrowRight className="h-3 w-3" /></Link>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-[1.4fr_1fr] gap-10">
        <section>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <Link to="/my-programmes/sem-2027/programme/s/$sessionId" params={{ sessionId: currentId }} className="h-11 px-6 inline-flex items-center bg-signal text-bone text-[11px] uppercase tracking-[0.28em] font-display">Start session</Link>
            <Link to="/my-programmes/sem-2027/programme/s/$sessionId" params={{ sessionId: currentId }} className="h-11 px-6 inline-flex items-center border border-border text-bone text-[11px] uppercase tracking-[0.28em] font-display">View detail</Link>
            <PdfDownloadLink slug="sem-2027" href={pdfAsset.url} label="PDF" />
          </div>

          <div className="mt-10">
            <p className="eyebrow mb-3">Readiness</p>
            <div className="flex gap-2">
              {(["ready", "average", "heavy"] as Sem27Readiness[]).map((r) => (
                <button key={r} onClick={() => sem27Store.setReadiness(currentId, r)}
            ...
            {wk.sessions.map((s) => {
              ...
              return (
                <li key={id} className="py-3 flex items-center justify-between gap-3">
                  ...
                  <Link to="/my-programmes/sem-2027/programme/s/$sessionId" params={{ sessionId: id }} className="text-foreground-muted hover:text-bone"><ArrowRight className="h-4 w-4" /></Link>
                </li>
              );
            })}
          </ul>

...
import { findSession, isCore } from "@/lib/sem2027/manifest";

export type Sem27Progress = {
  loading: boolean;
 ...
 * scoped to the S.E.M. 2027 product. The user_id filters are deliberate
 * defence-in-depth in addition to RLS.
 */
export function useSem27Progress(userId: string | undefined, productSlug = "sem-2027") {
  const [state, setState] = useState<Sem27Progress>({

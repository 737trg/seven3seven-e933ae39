...
import { findSession, isCore } from "@/lib/hrp/manifest";

export type HrpProgress = {
  loading: boolean;
 ...
 * scoped to the HYBRID RACE PLAN product. The user_id filters are deliberate
 * defence-in-depth in addition to RLS.
 */
export function useHrpProgress(userId: string | undefined, productSlug = "hybrid-race-plan") {
  const [state, setState] = useState<HrpProgress>({

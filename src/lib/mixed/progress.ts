...
import { findSession, isCore } from "@/lib/mixed/manifest";

export type MixedProgress = {
  loading: boolean;
...
const EMPTY: MixedProgress = {
  loading: false, coreCompleted: 0, optionalCompleted: 0, resultsLogged: 0, perSession: {},
};

/** Real session_completions + workout_results for the MIXED product. */
export function useMixedProgress(userId: string | undefined, productSlug = "mixed") {
  const [state, setState] = useState<MixedProgress>({ ...EMPTY, loading: true });
